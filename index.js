...
input.addEventListener("submit", function(event){
    ...
    fetch(
        ...
        .then(res => res.json())
        .then(data => {
            //data.colors.forEach(hex => colorsArr.push(hex.hex.value))
            colorsArr = data.colors.map(hex => hex.hex.value)
            console.log(colorsArr)
            renderData()
...
function renderData() {
  for(let i = 0; i < colorsArr.length; i++){
    document.getElementById(`hex-${[i]}`).innerText = colorsArr[i];
    document.getElementById(`color-${[i]}`).style.backgroundColor = colorsArr[i];
  }
}
